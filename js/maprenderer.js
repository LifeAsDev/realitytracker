...
class MapRenderer {
	...
	constructor() {
		downloadManager.download(
			`https://mapgallery.realitymod.com/json/levels.json`,
			"json",
			(data) => {
	...
	draw(ctx) {
		console.log("MapRenderer draw called");
		if (
			this.shouldUseFallback || // We failed downloading from map gallery, use old method
			(options_UseFallbackRenderer && MapImage != null)
		)
			// We selected the fallback renderer, but the fallback failed. Use new method.
		...
		const maxx = clamp(
			0,
			Math.floor((-CameraX + Canvas.width / options_canvasScale) / renderSize) +
				1,
			segmentCount
		);
		const maxy = clamp(
			0,
			Math.floor(
				(-CameraY + Canvas.height / options_canvasScale) / renderSize
			) + 1,
			segmentCount
		);
		...
		for (let x = minx; x < maxx; x++)
			for (let y = miny; y < maxy; y++) {
				...
				ctx.drawImage(
					img,
					CameraX + x * renderSize,
					CameraY + y * renderSize,
					renderSize,
					renderSize
				);
			}
	...
	_downloadSegment(zoom, x, y) {
		...
		const URL = `https://mapgallery.realitymod.com/images/maps/${
			this.mapname
		}/tiles/${zoom + 1}/${x}/${y}.jpg`;
	...
	_decideZoomlevel() {
		...
		return clamp(
			0,
			Math.floor(Math.log2(actualZoom * 0.8)),
			this.getMaxZoomCount() - 1
		);
	}
