...
function update() {
	...
	if (lastUpdateTime == null) lastUpdateTime = now - 10;
	...
	if (animations.animationsPlaying() || isPlaying) requestUpdate();

	if (!updateRequested) lastUpdateTime = null;
}
...
function updateLogic(frameTime) {
	...
	while (interpolation_CurrentAmount >= 1.0) {
		if (!Update())
			// If update returned false, don't touch anything anymore.
			return;
...
function onPlayBarChange() {
	const now = performance.now();
	if (now - lastPlayBarChange < playBarChangeCooldown) return;
...
function toggleSubMenu(submenu) {
	...
	if (menu.dialog("isOpen")) menu.dialog("close");
	else menu.dialog("open");
}
...
function onKeyDown(e) {
	...
	if ($("#addbookmarkComment").dialog("isOpen")) return;

	keysDown.add(e.which);
	requestUpdate();
	// 1 - 9
	if (e.which >= 49 && e.which <= 57) {
		if (e.shiftKey) selection_SelectSquad(1, e.which - 48);
		else selection_SelectSquad(2, e.which - 48);
	} else
		switch (e.which) {
			case 32: //space
				togglePlay();
			...
			case 71: //g
				...
				break;
			case 37: // left
				if (e.shiftKey) goTo(Tick_Current - TICKS_JUMP_AMOUNT_SHIFT);
				else goTo(Tick_Current - TICKS_JUMP_AMOUNT);
				break;
...
function updateHeader() {
	if (GameMode == "gpm_insurgency")
		$("#headerInner")[0].innerHTML =
			...
			tickets1 +
			" " +
			getTimeStringOfTick(Tick_Current) +
			" x" +
			playSpeed.toFixed(1);
	else
		$("#headerInner")[0].innerHTML =
...
function getTimeInSecondsFromString(time) {
	...
	if (match) return match[1] * 3600 + match[2] * 60 + match[3] * 1;
	return 0;
}
...
function setPlayBarBubble(e) {
	...
	var Tick = Math.max(
		Math.round((Position / e.target.clientWidth) * parseInt(e.target["max"])),
		0
	);
