...
function selection_SelectObject(obj) {
	if (obj == null) selection_SelectPlayer(SELECTED_NOTHING);
	else if (obj instanceof PlayerObject) selection_SelectPlayer(obj.id);
...
function selection_selectObjectSquad(object) {
	if (object != null) {
		if (object instanceof PlayerObject) selection_SelectPlayersSquad(object.id);
...
function selection_SelectVehicle(i) {
	...
	else selection_SelectPlayer(v.Passengers.values().next().value, true);
}
...
function selection_CheckIfPlayerInSelectedVehicle(i) {
	...
	else playerRows[i].classList.remove("player-row-selected");
}

function selection_SelectPlayersSquad(i) {
	if (i != SELECTED_NOTHING && i in AllPlayers) {
		var p = AllPlayers[i];
		if (p.team != 0 && p.squad != 0) selection_SelectSquad(p.team, p.squad);
	}
}
...
function selection_SelectSquad(Team, Squad) {
	...
	// Select new squad
		...
		$("#Squad" + Team + Squad)[0].classList.add(
...
function selection_UpdateInformationBox() {
	...
	// If player is selected
		...
		if (p.squad == 0) {
			if (p.isSquadLeader) div.innerHTML += "Commander";
			else div.innerHTML += "Unassigned";
		} else {
			div.innerHTML += "Squad " + p.squad;
			if (p.isSquadLeader) div.innerHTML += " Leader";
		}
		...
		if (p.health > 0) div.innerHTML += "<br>Health: " + p.health;

		if (p.team == 1) div.classList.add("color_Team1");
		else div.classList.add("color_Team2");
	}
}
