...
class GameObject {
	...
	getTeamColor3d() {
		switch (this.team) {
			...
			case 1:
				return renderer3d.red;
			case 2:
				return renderer3d.blue;
		}
	}
	getGeometry() {
...
class PlayerObject extends InterpolatedGameObject {
	...
	setKitImage() {
		if (this.kit != "") {
			...
			if (
				KitSubName in KitNameToImageDictionary &&
				KitNameToImageDictionary[KitSubName] in icons
			)
				...
				if (!this.ns_kitImage)
					console.error(
						"Parser: `rifleman` image in kitname-to-image dictionary must exist as fallback. Check the build script."
					);
			}
		}
	}
	getTeamColor3d() {
		if (SelectedPlayer == this.id) color = renderer3d.white;
		else if (
			this.team == SelectedSquadTeam &&
			this.squad == SelectedSquadNumber
		)
			return renderer3d.green;
		else return this.team == 1 ? renderer3d.red : renderer3d.blue;
	}
}
...
const playerUpdate_TeamOrSquadChange =
	PLAYERUPDATEFLAGS.TEAM + PLAYERUPDATEFLAGS.SQUAD;

// When to rewrite selection
const playerUpdate_SelectionInfoChange =
	playerUpdate_StatusChange + PLAYERUPDATEFLAGS.HEALTH;
...
function PlayerUpdate(FullMessage) {
	...
	while (pos != length) {
		...
		if (flags & PLAYERUPDATEFLAGS.POSITION) {
			const x = FullMessage.getInt16(pos, true);
			const y = FullMessage.getInt16(pos + 2, true);
			const z = FullMessage.getInt16(pos + 4, true);

			if (!Number.isNaN(x)) Player.X = x;
			if (!Number.isNaN(y)) Player.Y = y;
			if (!Number.isNaN(z)) Player.Z = z;
			if (Player.isAlive) {
				deathPoints.push({ X: Player.X, Y: Player.Y, Z: Player.Z });
			}
			pos += 6;
		}
		...
		if (InitialParse) continue;

		// Reset interpolation if alive changed
		if (flags & PLAYERUPDATEFLAGS.ISALIVE) Player.updateInterpHistory();

		// Call events for each player
		if (flags & playerUpdate_StatusChange) onPlayerStatusChanged(id);

		if (flags & playerUpdate_TeamOrSquadChange) onPlayerTeamOrSquadChange(id);
...
class VehicleObject extends InterpolatedGameObject {
	constructor(id) {
		...
		for (var i in AllPlayers) {
			...
			if (p.vehicleid == this.id) this.Passengers.add(i);
		}
	...
	getTeamColor3d() {
		if (SelectedVehicle == this.id) return renderer3d.white;
		else if (this.id in SquadVehicles) return renderer3d.green;
		else return this.team == 1 ? renderer3d.red : renderer3d.blue;
	}
	...
	initializeGraphics() {
		...
		if (this.name in vehicleData) {
			...
			if (data.MenuIcon != "") this.ns_menuImage = icons[data.MenuIcon]; //Menuimage is optional
			this.ns_geomName = data.Geometry;
		} else console.log("Parser: Vehicle Name not in dictionary: " + this.name);
	}
...
function VehicleAdd(FullMessage) {
	...
	var CurrentVehicle;
	if (id in AllVehicles) CurrentVehicle = AllVehicles[id];
	else CurrentVehicle = new VehicleObject(id);
...
function VehicleUpdate(FullMessage) {
	...
	while (pos != length) {
		...
		if (flags & VEHICLEUPDATEFLAGS.HEALTH) {
			...
			if (!isFastForwarding)
				CurrentVehicle.ns_healthRenderer.onDamage(
					CurrentVehicle,
		...
		if (isNaN(CurrentVehicle.ns_lastX)) CurrentVehicle.updateInterpHistory();

		// First update received for vehicle, do UI stuff
		if (CurrentVehicle.isNew) {
			CurrentVehicle.isNew = false;
			if (!InitialParse) onNewVehicle(id);
		}
	}
}
...
function RecheckAllVehicles() {
	for (var vid in AllVehicles) AllVehicles[vid].Passengers = new Set();
...
function FlagUpdate(FullMessage) {
	...
	const parsed = unpacked[1];
	if (parsed[0] in AllFlags) AllFlags[parsed[0]].team = parsed[1];
}
...
function RallyRemove(FullMessage) {
	...
	if (TeamSquad in AllRallies) delete AllRallies[TeamSquad];
}
...
function FoBRemove(FullMessage) {
	...
	while (pos != length) {
		...
		if (parsed[0] in AllFobs) delete AllFobs[parsed[0]];
	}
}
...
function CacheAdd(FullMessage) {
	...
	while (pos != length) {
		...
		new CacheObject(
			id, //Cache ID (The module numbers them)
			FullMessage.getInt16(pos + 1, true), //X
			FullMessage.getInt16(pos + 3, true), //Y
			FullMessage.getInt16(pos + 5, true)
		); //Z
		pos += 7;
	}
}
...
class ProjObject extends InterpolatedGameObject {
	...
	getTeamColor3d() {
		const p = this.player;
		if (p == null) return super.getTeamColor3d();
...
function ProjUpdate(FullMessage) {
	...
	while (pos != length) {
		...
		proj.ns_isFast =
			proj.ns_isFast |
			(Math.pow(proj.X - proj.ns_lastX, 2) +
				Math.pow(proj.Z - proj.ns_lastZ, 2) >
				1600 * DemoTimePerTick); // 40m/s
	}
}
...
function SquadName(FullMessage) {
	...
	while (pos != length) {
		...
		if (!(team in SquadNames)) SquadNames[team] = {};

		if (!(squad in SquadNames[team])) SquadNames[team][squad] = {};
		SquadNames[team][squad] = name;

		if (!InitialParse) onSquadNameChanged(team, squad, name);
	}
}

function getSquadName(team, squad) {
	if (!(team in SquadNames)) return "";
	if (!(squad in SquadNames[team])) return "";
...
function goTo(Tick_Target) {
	...
	if (Tick_Target == Tick_Current) return;
	// if the bar was dragged below first tick, load the first tick.
	if (Tick_Target < 1) Tick_Target = 1;
	// if its after the last tick, load the last state
	if (Tick_Target >= Tick_Count) Tick_Target = Tick_Count - 1;
	...
	if (Tick_Target > Tick_Current) {
		...
		else if (LatestState < Tick_Target) loadState(LatestState);
	...
	while (Tick_Current < Tick_Target - 1) Update();

	isFastForwarding = false;
	if (Tick_Current < Tick_Target)
		// If no updates are needed, the UI should be already updated from loading the state
...
function loadState(SavedTick) {
	...
	for (var handler in eventArrays) eventArrays[handler].loadState(State);
...
function saveState() {
	...
	for (var handler in eventArrays) eventArrays[handler].saveState(state);
...
function Reset() {
	...
	for (var handler in eventArrays) eventArrays[handler].reset();
...
function Update() {
	...
	// Interpolation
	if (!isFastForwarding) {
		// Do not interpolate when fast forwarding
		[AllPlayers, AllVehicles, AllProj].forEach((dict) => {
			for (var key in dict) dict[key].updateInterpHistory();
		});
	}
	...
	if (InitialParse) Tick_Count++;

	// If we're doing initial parse or if this is live demo, save states.
	if (InitialParse)
		if (
			Tick_Current % TICKSPERSAVE == 0 &&
			!(Tick_Current in savedStates) &&
			Tick_Current != 0
		)
...
function DODList(FullMessage) {
	...
	while (pos != length) {
		...
		for (var i = 0; i < numOfPoints; i++) {
			const point = [
				FullMessage.getFloat32(pos, true),
				FullMessage.getFloat32(pos + 4, true),
			];
			pos += 8;
...
function ServerDetails(FullMessage) {
	...
	if (Version != CURRENTVERSION)
		console.log(
			"Wrong version detected. Expected: " +
				CURRENTVERSION +
				", file is " +
				Version
		);
	...
	if (pos == FullMessage.byteLength) return;
...
function eventArray(typeCode) {
	...
	this.onIteratorIncreasedByOne = (obj) =>
		console.log("onIteratorIncreasedByOne missing"); //(UI) Mark the new one as not future
...
$(() => {
	...
	eventArrays.kills.onMessage = (FullMessage) => {
		...
		} else console.log("Received Attacker ID of invalid player");

		if (VictimObj != null) {
			obj.VictimName = VictimObj.name;
			obj.VictimTeam = VictimObj.team;
		} else console.log("Received Victim ID of invalid player");
	...
	eventArrays.kills.onNewEvent = (obj) => {
		eventDialogs.kills.newEvent(obj);
	};
	eventArrays.kills.onIteratorIncreasedByOne = (obj) => {
		killLine_Add(obj);
		eventDialogs.kills.enableOne();
	};
	eventArrays.kills.onLoadState = (i) => {
		eventDialogs.kills.reenable(i);
	};
});
...
$(() => {
	...
	eventArrays.revives.onMessage = (FullMessage) => {
		...
		if (medicObj != null) {
			...
			obj.X = medicObj.X;
			obj.Y = medicObj.Z;
		} else console.log("Received Medic ID of invalid player");

		if (revivedObj != null) obj.RevivedName = revivedObj.name;
		else console.log("Received Revived ID of invalid player");

		return obj;
	};
	eventArrays.revives.onNewEvent = (obj) => eventDialogs.revives.newEvent(obj);
	eventArrays.revives.onIteratorIncreasedByOne = () =>
		eventDialogs.revives.enableOne();
	eventArrays.revives.onLoadState = (i) => {
		eventDialogs.revives.reenable(i);
	};
});
...
$(() => {
	...
	eventArrays.vehicleDestroyers.onNewEvent = (obj) =>
		eventDialogs.vehicleDestroyers.newEvent(obj);
	eventArrays.vehicleDestroyers.onIteratorIncreasedByOne = (obj) => {
		delete AllVehicles[obj.vehicleID];
		onVehicleDestroyed(obj.vehicleID);
		eventDialogs.vehicleDestroyers.enableOne();
	};
	eventArrays.vehicleDestroyers.onLoadState = (i) => {
		eventDialogs.vehicleDestroyers.reenable(i);
	};
});
...
$(() => {
	...
	eventArrays.chat.onMessage = (FullMessage) => {
		...
		if (channel < 0x30) {
			speakerName = AllPlayers[playerID]
	...
	eventArrays.chat.onNewEvent = (obj) => {
		eventDialogs.chat.newEvent(obj);
	};
	eventArrays.chat.onIteratorIncreasedByOne = (obj) => {
		eventDialogs.chat.enableOne();
	};
	eventArrays.chat.onLoadState = (i) => {
		eventDialogs.chat.reenable(i);
	};
});
...
$(() => {
	...
	eventArrays.kitAllocations.onMessage = (FullMessage) => {
		...
		const playerName = AllPlayers[playerID]
			? AllPlayers[playerID].name
			: "Unknown Player!";
	...
	eventArrays.kitAllocations.onNewEvent = (obj) => {
		eventDialogs.kitAllocations.newEvent(obj);
	};
	eventArrays.kitAllocations.onIteratorIncreasedByOne = (obj) => {
		eventDialogs.kitAllocations.enableOne();
	};
	eventArrays.kitAllocations.onLoadState = (i) => {
		eventDialogs.kitAllocations.reenable(i);
	};
});
...
function messageArray() {
	...
	this.getMessageAt = function (i) {
		if (this.messages.length > i) return this.messages[i];
		else return null;
	};
	...
	this.getNextMessage = function () {
		...
		if (this.pos + 2 > endOfBuffer) return null; // End of file reached
		...
		if (this.pos + 2 + Length > endOfBuffer) return null; // Error, unexpected end of file
	...
	this.updateNewMessages = function () {
		...
		if (messageType != MESSAGETYPE.ROUNDEND) {
			console.log(
				"Unexpected file end. Removing last message as it may be incomplete"
			);
			this.messages.pop();
		}
	};
	...
	this.addMessage = function (message) {
		...
		if (
			messageType == MESSAGETYPE.PROJ_UPDATE ||
			messageType == MESSAGETYPE.PROJ_ADD ||
			messageType == MESSAGETYPE.PROJ_REMOVE
		) {
...
function isClimbingVehicle(vehicleName) {
	return (
		vehicleName == "grapplinghookropecontainer" ||
...
function loadList(Source, Target) {
	for (var objid in Target) if (!(objid in Source)) delete Target[objid];

	for (var field in Source) {
		if (field in Target) shallowCopy(Source[field], Target[field]);
		else Target[field] = shallowCopy(Source[field]);
...
function shallowCopy(Source, Target) {
	if (!Target) Target = Object.create(Source.__proto__);

	for (var field in Source) {
		if (field.startsWith("ns_")) continue;
...
var messageHandlers = [];
$(() => {
	for (var i = 0; i < 256; i++) messageHandlers[i] = unknownMessage;
	...
	messageHandlers[MESSAGETYPE.VEHICLE_DESTROYED] =
		eventArrays.vehicleDestroyers.message;
	...
	messageHandlers[MESSAGETYPE.KITALLOCATED] =
		eventArrays.kitAllocations.message;
