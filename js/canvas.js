...
function interpolate(Start, End) {
	return (
		Start * (1 - interpolation_CurrentAmount) +
		End * interpolation_CurrentAmount
	);
}

function extrapolate(Start, End, extra) {
	return (
		Start * (1 - interpolation_CurrentAmount - extra) +
		End * (interpolation_CurrentAmount + extra)
	);
}

function interpolateAngle(Start, End) {
	if (End - Start > 180) End -= 360;
	else if (Start - End > 180) Start -= 360;
...
class Renderer2d {
	...
	mouseClick(pos) {
		...
		for (var I in AllPlayers) {
			...
			const x = p.getX();
			const z = p.getZ();
			if (p.isJoining) continue;

			var dis =
				Math.pow(XtoCanvas(x) - pos.X, 2) + Math.pow(YtoCanvas(z) - pos.Y, 2);
			if (dis < minDis) {
				objectToSelect = p;
				minDis = dis;
			}
		...
		for (var I in AllProj) {
			...
			const z = p.getZ();
			var dis =
				(Math.pow(XtoCanvas(x) - pos.X, 2) +
					Math.pow(YtoCanvas(z) - pos.Y, 2)) *
				2; // less click priority than players
			if (dis < minDis) {
				objectToSelect = p;
				minDis = dis;
			}
	...
	draw() {
		var gametime =
			(Tick_Current + interpolation_CurrentAmount) * DemoTimePerTick;
		gametimepassed = Math.max(
			0,
			gametime - (isNaN(gametimelasttime) ? gametime : gametimelasttime)
		);
		...
		//Draw Flags
		Context.lineWidth = 1;
		for (var i in AllFlags) drawFlag(i);

		//Draw Rallies
		Context.font = "bold 26px Arial";
		Context.strokeStyle = "green";
		Context.lineWidth = 2;
		for (var i in AllRallies) drawRally(i);
		...
		for (var i in AllFobs) drawFob(i);

		SquadVehicles = {};
		Context.font = "bold 12px Arial";
		for (var i in AllPlayers) if (i != SelectedPlayer) drawPlayer(i);

		for (var i in AllVehicles) drawVehicle(i);

		for (var i in AllCaches) drawCache(i);
		for (var i in AllProj) drawProj(i);

		//Draw selected player above everyone else
		if (SelectedPlayer != SELECTED_NOTHING) drawPlayer(SelectedPlayer);

		//Draw selected kill line
		if (SelectedKill != SELECTED_NOTHING) killLine_DrawSelected();

		if (options_drawAllOrderIcons)
			for (var index in AllSLOrders) drawOrderIcon(AllSLOrders[index]);
...
function drawOrderIcon(order) {
	if (order == null || order.type == -1) return;
...
function drawPlayer(index) {
	...
	if (p.isJoining || p.ns_lastX == null)
		//Skip if joining or unknown pos
		return;
	...
	//Decide circle size
	if (options_DrawKitIcons || p.isSquadLeader) PlayerCircleSize = 8;
	else PlayerCircleSize = 6;

	Context.beginPath();

	//In vehicle (that is not a climbing vehicle), has to be alive
	if (p.vehicleid >= 0 && !isVehicleContainer(p.vehicleid)) {
		if (isSquadSelected) SquadVehicles[p.vehicleid] = true;
			...
			case 0:
				...
				if (p.vehicleid == SelectedVehicle)
					drawFieldOfView(
						...
						y,
						1 * CameraZoom,
						driverFOVSize_selected * CameraZoom,
						rot,
						driverFOVArc_selected
					);
				else
					drawFieldOfView(
						x,
						y,
						1 * CameraZoom,
						driverFOVSize * CameraZoom,
						rot,
						driverFOVArc
					);
				break;
			case 1:
				if (!gunnerRotationSupported)
					// TODO for demos before 1.4.21. remove for version 4
					break;
				...
				if (p.vehicleid == SelectedVehicle)
					drawFieldOfView(
						...
						y,
						1 * CameraZoom,
						gunnerFOVSize_selected * CameraZoom,
						rot,
						gunnerFOVArc_selected
					);
				else
					drawFieldOfView(
						x,
	...
	if (isPlayerSelected || (isSquadSelected && p.isSquadLeader)) {
		...
		if (!options_drawAllOrderIcons) drawOrderIconByTeamSquad(p.team, p.squad);
	}
}

document.addEventListener("DOMContentLoaded", () => {
	function convertDeathPointsToHeatmapData() {
		const mapWidth = Canvas.width; // Ancho del canvas
		const mapHeight = Canvas.height; // Alto del canvas

		return {
			max: 70, // Intensidad máxima
			data: deathPoints.map((p) => {
				// Convertir las coordenadas del mundo al sistema de coordenadas del canvas
				const x = Math.floor(XtoCanvas(p.X));
				const y = Math.floor(YtoCanvas(p.Z));

				return {
					x: x, // Coordenada X en el heatmap
					y: y, // Coordenada Y en el heatmap
	...
	const mapDiv2 = document.getElementById("mapDiv2");

	const heatmapInstance = h337.create({
		container: mapDiv2, // Contenedor donde se renderizará el heatmap
		radius: 5, // Radio de los puntos de calor
		maxOpacity: 0.4, // Opacidad máxima
		minOpacity: 0, // Opacidad mínima
		blur: 0.75, // Nivel de desenfoque
	});
	// Generar el heatmap
	function generateHeatmap() {
		const heatmapCanvas = mapDiv2.querySelector("canvas");

		const heatMapCtx = heatmapCanvas.getContext("2d");

		const heatmapData = convertDeathPointsToHeatmapData();
		heatMapCtx.clearRect(0, 0, heatmapCanvas.width, heatmapCanvas.height);

		/* 		syncHeatmapWithCanvas();
		 */ /* 		console.log(JSON.stringify(heatmapData));
		 */ heatmapInstance.setData(heatmapData);

		console.log("Heatmap generado.");
	}

	// Escuchar eventos de teclado
	document.addEventListener("keydown", (event) => {
		if (event.key === "O" || event.key === "o") {
			console.log("Generando heatmap...");
			generateHeatmap();
		}
		if (event.key === "L" || event.key === "l") {
			...
			if (heatmapCanvas) {
				const heatMapCtx = heatmapCanvas.getContext("2d");
				console.log({ CameraX, CameraY, CameraZoom, heatMapCtx });
				heatMapCtx.scale(CameraZoom, CameraZoom);

				heatMapCtx.setTransform(
					CameraZoom * options_canvasScale, // Escala en X
					0, // Sesgo en Y
					0, // Sesgo en X
					CameraZoom * options_canvasScale, // Escala en Y
					CameraX, // Desplazamiento en X
					CameraY // Desplazamiento en Y
				);
			}
		}
	});

	console.log("Heatmap.js inicializado y listo.");
});

function syncHeatmapWithCanvas() {
	const mapDiv2 = document.getElementById("mapDiv2");

	if (!mapDiv2) {
		console.error("Canvas o mapDiv2 no encontrados.");
		return;
	}

	// Alinear el contenedor del heatmap con el canvas
	const rect = Canvas.getBoundingClientRect();
	mapDiv2.style.position = "absolute";
	mapDiv2.style.top = `${rect.top}px`;
	mapDiv2.style.left = `${rect.left}px`;
	mapDiv2.style.width = `${rect.width}px`;
	mapDiv2.style.height = `${rect.height}px`;
	mapDiv2.style.pointerEvents = "none"; // No interferir con clics
	mapDiv2.style.zIndex = "10"; // Asegurarse de que esté encima del canvas
}
...
function drawPlayer_DrawDead(p, x, y) {
	const outerstyle = p.isSquadLeader
		? Style_SquadLeaderStroke
		: Style_GruntStroke;
	var teamstyle;
	...
	else teamstyle = p.team == 2 ? Style_BlueTeam : Style_RedTeam;
...
function drawPlayer_DrawAlive(p, x, y, rot) {
	...
	if (p.team == SelectedSquadTeam && p.squad == SelectedSquadNumber) {
		...
		else drawPlayer_DrawNormal(p, x, y, Style_SquadSelection, outerstyle);
	} else if (p.id == SelectedPlayer)
		//is selected
		drawPlayer_DrawSelected(p, x, y, teamstyle);
	else drawPlayer_DrawNormal(p, x, y, teamstyle, outerstyle);
	...
	if (options_DrawKitIcons) drawPlayer_DrawKit(p, x, y);
}
...
function drawPlayer_DrawWithInnerStroke(
	p,
	x,
	y,
	FillStyle,
...
function drawPlayer_DrawHollowWithInnerStroke(
	p,
	x,
	y,
	OuterStrokeStyle,
...
class HealthRenderer {
	...
	draw(x, y) {
		...
		if (p.health == HEALTH_ERROR) return;
		...
		if (p.health == HEALTH_WRECK) {
			...
			if (this.incomingDamage > 0)
				this.incomingDamage = Math.max(
					0,
					this.incomingDamage -
						gametimepassed *
							(DAMAGEINCOMING_PERCENT_PERSEC_CONST +
								this.incomingDamage * DAMAGEINCOMING_PERCENT_PERSEC_MUL)
				);
...
function drawFob(i) {
	...
	if (fob.team == 2) Context.fillStyle = "blue";
...
function drawVehicle(i) {
	...
	if (!options_DrawEmptyVehicles && v.Passengers.size == 0) return;
	//or if is a climbing/container vehicle, skip
	if (isVehicleContainer(i)) return;
	...
	if (i == SelectedVehicle) color = 3;
	else if (i in SquadVehicles) color = 2;
	else if (v.team != 0) color = v.team - 1;
	else color = 0; // TODO neutral vehicles color
	...
	Context.save();
	Context.translate(x, y);
	if (!v.isUAVVehicle) Context.rotate((rot / 180) * Math.PI);
...
function drawRally(i) {
	...
	if (R.team == 2) Context.fillStyle = "blue";
	else Context.fillStyle = "red";
...
function drawCache(i) {
	...
	else Context.drawImage(icons.CacheUnrevealed, x - 13, y - 13, 26, 26);
}
...
function drawProj(i) {
	...
	if (
		proj.ns_isFast &&
		Math.abs(proj.ns_lastX - proj.X) < 10 &&
	...
	if (proj.ns_icon != null) {
		...
		Context.translate(x, y);
		if (proj.ns_shouldRotate) Context.rotate((proj.rotation / 180) * Math.PI);
